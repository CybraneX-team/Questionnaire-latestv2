...
const ChatBot = ({ isPdfOpen }) => {
  ...
  const handleSendMessage = async () => {
    if (inputMessage.trim() !== '') {
      ...
      try {
        ...
        // Update chat history
        setChatHistory(prevHistory => [...prevHistory, [inputMessage, response.data.answer]]);
      } catch (error) {
  ...
  const handleFileUpload = async (event) => {
    ...
    if (validFiles.length !== files.length) {
      setMessages(prev => [...prev, { text: "Only PDF files are allowed.", sender: 'system' }]);
    } else {
      ...
      try {
        ...
        setMessages(prev => [...prev, { text: "Sorry, I couldn't upload the files.", sender: 'bot' }]);
      }
    }
  };
